...
@PerActivity
public class BrandListPresenter implements Presenter {

    private BrandListView viewListView;

    private final UseCase getBrandListUseCase;
    private final ModelDataMapper brandModelDataMapper;

    @Inject
    public BrandListPresenter(@Named("brandList") UseCase getBrandListUseCase, ModelDataMapper brandModelDataMapper) {
        this.getBrandListUseCase = getBrandListUseCase;
    ...
    @Override
    public void destroy() {
        this.getBrandListUseCase.unsubscribe();
    ...
    private void getBrandList() {
        this.getBrandListUseCase.execute(new BrandListSubscriber());
    }

    private final class BrandListSubscriber extends DefaultSubscriber<List<Brand>> {

        @Override
        public void onCompleted() {
            BrandListPresenter.this.hideViewLoading();
        }

        @Override
        public void onError(Throwable e) {
            BrandListPresenter.this.hideViewLoading();
            BrandListPresenter.this.showErrorMessage(new DefaultErrorBundle((Exception) e));
            BrandListPresenter.this.showViewRetry();
        }

        @Override
        public void onNext(List<Brand> brands) {
            BrandListPresenter.this.showBrandsCollectionInView(brands);
        }
    }
}
